...
def profile_overview(user_id):
    ...
    base = {
        ...
        "employee": None,
        "stats": {"leave_balance": None, "attendance": None, "salary": None, "performance": None}, "documents": [], "payrolls": [], "reviews": [],
    }
    ...
    base["documents"] = [{"id": item.id, "type": item.document_type, "name": item.document_name, "issue_date": item.issue_date.isoformat() if item.issue_date else None, "expiry_date": item.expiry_date.isoformat() if item.expiry_date else None} for item in employee.documents if item.deleted_at is None]
    base["payrolls"] = [{"id": item.id, "period": f"{item.payroll_period.start_date.isoformat()} to {item.payroll_period.end_date.isoformat()}", "basic": float(item.basic_salary), "bonus": float(item.bonus), "deductions": float(item.total_deduction + item.tax_amount), "net": float(item.net_salary), "status": item.payment_status} for item in sorted(employee.payrolls, key=lambda value: value.created_at, reverse=True)[:12]]
    base["reviews"] = [{"id": item.id, "date": item.review_date.isoformat(), "reviewer": f"{item.reviewer.first_name} {item.reviewer.last_name}".strip(), "score": float(item.score) if item.score is not None else None, "comments": item.comments, "status": item.status} for item in sorted((review for review in employee.performance_reviews if review.deleted_at is None), key=lambda value: value.review_date, reverse=True)]
    return base
