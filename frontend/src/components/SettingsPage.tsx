...
const fallback: UserSettings = {
  profile: { name: "", email: "", phone: null, job_title: null, role: null },
  appearance: { theme: "system", density: "comfortable" },
  notifications: { email: true, push: true, leave: true, payroll: true },
};
export default function SettingsPage({
  ...
  useEffect(() => {
    getUserSettings()
      .then((value) => {
        ...
        const resolved = local ? { ...value, appearance: local } : value;
        setSettings(resolved);
        setOriginal(resolved);
        applyAppearance(resolved.appearance);
      })
      .catch((e) =>
  ...
  const discard = () => {
    ...
    applyAppearance(original.appearance);
    setCurrent("");
    setNext("");
    setConfirm("");
    setError("");
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      if (tab === "profile") {
        ...
        value = { ...response, appearance: settings.appearance };
        if (user)
          onUserUpdated({
            ...user,
        ...
        value = await updateAppearance(settings.appearance);
  ...
  const tabs: Array<[Tab, string, typeof UserRound]> = [
    ["profile", t("tabs.profile"), UserRound],
    ["appearance", t("tabs.appearance"), Palette],
    ["notifications", t("tabs.notifications"), Bell],
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[230px_1fr]">
        <nav className="h-fit rounded-2xl border bg-white p-2 shadow-sm">
          {tabs.map(([value, label, Icon]) => (
            <button
              ...
              className={cn(
                "flex h-12 w-full items-center gap-3 rounded-xl px-4 font-bold",
                tab === value
        ...
        >
          <section className="p-8">
            {tab === "profile" && (
              <>
                <h3 className="text-xl font-bold">{t("tabs.profile")}</h3>
                <div className="mt-7 flex items-center gap-5">
                  <span className="flex h-20 w-20 items-center justify-center rounded-2xl bg-indigo-100 text-2xl font-bold text-indigo-600">
                ...
                <div className="mt-7 grid gap-5 md:grid-cols-2">
                  ...
                  <Field label={t("profile.jobTitle")}>
                    <input
                      ...
                      title="Job titles are managed by HR"
                    />
                  </Field>
                </div>
              </>
            ...
            {tab === "appearance" && (
              <>
                <h3 className="text-xl font-bold">{t("appearance.title")}</h3>
                <p className="mt-7 text-sm font-bold">{t("appearance.theme")}</p>
                <div className="mt-3 grid gap-4 sm:grid-cols-3">
                  {(
                    [
                      ["light", t("appearance.light"), Sun],
                      ["dark", t("appearance.dark"), Moon],
                      ["system", t("appearance.system"), Monitor],
                    ] as const
                    ...
                    <button
                      ...
                      className={cn(
                        "flex h-24 flex-col items-center justify-center rounded-xl border-2 font-bold uppercase",
                ...
                <p className="mt-7 text-sm font-bold">{t("appearance.density")}</p>
                  ...
                  {(
                    [
                      [
                        ...
                        t("appearance.comfortableDescription"),
                        LayoutPanelTop,
                      ],
                      ["compact", t("appearance.compact"), t("appearance.compactDescription"), Monitor],
                    ] as const
                  ).map(([value, label, note, Icon]) => (
                    <button
                      type="button"
                      aria-pressed={settings.appearance.density === value}
                      key={value}
                      onClick={() => {
                        const appearance = {
                          ...settings.appearance,
                      ...
                      className={cn(
                        "flex h-20 items-center gap-4 rounded-xl border-2 px-5 text-left",
            ...
            {message && (
              <p className="mt-5 flex items-center gap-2 rounded-xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-700">
                <Check className="h-4 w-4" />
                {message}
              </p>
            )}
          </section>
          <footer className="flex justify-end gap-4 border-t bg-slate-50 p-5">
            <button
              type="button"
              onClick={discard}
...
function Field({
  ...
  return (
    <label className="block text-sm font-bold">
      {label}
      <span className="mt-2 block [&>input]:h-12 [&>input]:w-full [&>input]:rounded-xl [&>input]:bg-slate-50 [&>input]:px-4 [&>input]:font-normal [&>input]:outline-none [&>input:read-only]:cursor-not-allowed [&>input:read-only]:text-slate-500">
        {children}
      </span>
    </label>
  );
