    ...
    <motion.article layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="p-6">
        ...
        <div className="mt-6 grid grid-cols-2 gap-4">
          ...
          <div className="rounded-xl bg-slate-50 p-4"><p className="flex items-center gap-2 text-xs font-bold uppercase text-slate-500"><DollarSign className="h-4 w-4" />Budget</p><p className="mt-2 text-lg font-bold text-slate-950">{department.annual_budget === null ? 'Not set' : `$${department.annual_budget.toLocaleString()}`}</p></div>
        </div>
      </div>
...
export default function DepartmentsPage({ departmentId, onNavigate }: { departmentId?: number; onNavigate: (path: string) => void }) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-7">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-3xl font-bold text-slate-950">Department Management</h2><p className="mt-2 text-lg text-slate-500">Organize your company structure and teams.</p></div><button onClick={() => onNavigate('departments/new')} className="flex h-12 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 font-bold text-white shadow-lg shadow-indigo-200 transition hover:bg-indigo-700"><Plus className="h-5 w-5" />Add Department</button></div>
