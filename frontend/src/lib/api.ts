...
export type ProfileOverview={account:{id:number;name:string;email:string;role:string|null;is_active:boolean;created_at:string;last_login_at:string|null};employee:null|{id:number;employee_code:string;name:string;email:string|null;phone:string|null;personal_email:string|null;address:string|null;department:string|null;position:string|null;manager:string|null;status:string;employment_type:string;hire_date:string|null;probation_end_date:string|null;basic_salary:number;bank_name:string|null;bank_account_name:string|null;bank_account_number:string|null;emergency_contact_name:string|null;emergency_contact_phone:string|null;shift:string|null};stats:{leave_balance:number|null;attendance:number|null;salary:number|null;performance:number|null};documents:Array<{id:number;type:string;name:string;issue_date:string|null;expiry_date:string|null}>;payrolls:Array<{id:number;period:string;basic:number;bonus:number;deductions:number;net:number;status:string}>;reviews:Array<{id:number;date:string;reviewer:string;score:number|null;comments:string|null;status:string}>};
export function getProfileOverview(){return request<ProfileOverview>('/api/v1/settings/profile-overview');}
